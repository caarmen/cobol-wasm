...
if [ $# -eq 0 ]; then
  echo "Usage: $0 <path_to_cobol_file>"
  exit 1
fi
...
docker run --rm -it \
  ...
  -v "${input_dir}:/workspace/input" \
  "${docker_image}" \
  bash -c "cd /workspace/output && source /workspace/deps/emsdk/emsdk_env.sh && emcc -o ${base_name}.js -O3 -s WASM=1 -sEXPORTED_FUNCTIONS='[\"_ANSWER__TO__LIFE\", \"_cob_init\", \"_malloc\", \"_free\"]' -sEXPORTED_RUNTIME_METHODS='[\"cwrap\", \"HEAPU8\", \"UTF8ToString\"]' -I/opt/gnucobol-wasm/include -L/opt/gnucobol-wasm/lib -lcob -L/workspace/deps/gmp/.libs -lgmp /workspace/deps/libdb/build_unix/.libs/libdb-5.3.so ${base_name}.c"
